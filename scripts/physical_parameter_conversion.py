...
def physical_parameter_conversion(
    ...
    if drop_neg_nan:
        table = table[~table['kin_pa'].mask]
        table = table[table['kin_pa'] != -1]
    else:
        table['kin_pa'][table['kin_pa'].mask] = 0.0
        table['kin_pa'][table['kin_pa'] == -1] = 0.0
    ...
    # Replace stdout writing with data collection
    source_id_counter = 0
    ...
    for i in range(len(freq)):
        ...
        if any(
            isinstance(_d[i], np.ma.core.MaskedConstant)
            for _d in (ell_maj, ell_min, ell_pa, flux)
        ):
            continue
        ...
        if write_to_cat:
            data['id'].append(source_id_counter)
            data['ra'].append(ra[i])
            data['dec'].append(dec[i])
            data['hi_size'].append(hi_size)
            data['line_flux_integral'].append(flux[i] if no_corr else flux[i] / corr_flux(flux[i]))
            data['central_freq'].append(freq[i])
            data['pa'].append(kin_pa[i])
            data['i'].append(incl)
            data['w20'].append(w20[i])
            source_id_counter += 1
...
if __name__ == '__main__':
    import argh
    argh.dispatch_command(physical_parameter_conversion, old_name_mapping_policy=False)
